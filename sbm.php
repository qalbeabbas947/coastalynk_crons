...
$sql = "select * from ".$table_name." where port_type='Offshore Terminal'";
$candidates = [];
if ($result = $mysqli -> query($sql)) {
    ...
    foreach( $types as $type ) {
        while ( $obj = $result->fetch_object() ) {
            ...
            foreach ($vessels as $v1) {
                $sql = "select id, uuid, is_start_email_sent from ".$table_name_sbm." where is_offloaded='No' and uuid='".$mysqli->real_escape_string($v1['uuid'])."'";
                ...
                if( $num_rows == 0 ) {
                        ...
                        $sql = "INSERT INTO $table_name_sbm (uuid , name, mmsi, imo, country_iso, type, type_specific, lat, lon,speed,navigation_status, draught, last_position_UTC, distance, port, port_id, last_updated)
                        VALUES (
    ...
    $sql = "select * from ".$table_name_sbm." where is_offloaded='No'";
    $result = $mysqli->query($sql);
    $num_rows = mysqli_num_rows($result);
    if( count($array_ids) > 0 ) {
        $array_ids = implode(',', $array_ids);
        $sql = "select * from ".$table_name_sbm." where is_offloaded='No' and id not in (".$array_ids.")";
