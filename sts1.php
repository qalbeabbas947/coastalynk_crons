    ...
    if( count($array_ids) > 0 ) {
        ...
        if( $num_rows > 0 ) {
            while( $row = mysqli_fetch_array($result2, MYSQLI_ASSOC) ) {
                ...
                $v1_current_draught = get_datalastic_field($row['vessel1_uuid'], 'current_draught');
                $v2_current_draught = get_datalastic_field($row['vessel2_uuid'], 'current_draught');

                $sql = "update ".$table_name_sts." set is_complete = 'Yes', vessel1_completed_draught = '".$v1_current_draught."', vessel2_completed_draught = '".$v2_current_draught."', end_date = NOW() where is_complete='No' and id='".$row['id']."'";
                $mysqli->query($sql);
            }
        }
    }
