...
$sql = "CREATE TABLE IF NOT EXISTS `".$event_table_daughter."` (
  ...
  `outcome_status` varchar(30) NOT NULL DEFAULT '',
  `flag_status` varchar(30) NOT NULL DEFAULT '',
...
$total_allowed = 0;
echo '<br>Total vessels:'.$data['data']['total'].'\n';
if( isset( $data ) && isset( $data['data'] ) && isset( $data['data']['total'] ) && intval( $data['data']['total'] ) > 0 ) {
    ...
    foreach ($vessels as $v1) {
        ...
        $vessel_array = [$v1['mmsi'] => ['uuid'=> $v1['uuid'], 'name'=> $v1['name']]];
        if( !empty( $v1['type'] ) && str_contains( $v1['type'], 'Tanker' ) ) {
            ...
            foreach ($vessels as $v2) {
                ...
                if ($v1['uuid'] != $v2['uuid'] && !empty( $v2['type'] ) && str_contains($v2['type'], 'Tanker')) { // && $total_allowed < 1
                    ...
                    if( $num_rows == 0 ) {
                        ...
                        $at_event_creation_allowed_heading = false;
                        
                        if( $result = $mysqli->query($sql) ) {
                            $distance_obj = $result->fetch_object();
                           // echo '<br>'.$distance_obj->distance_meters.' <= '.$distance_threshold.' = '.($distance_obj->distance_meters <= $distance_threshold ? 'true' : '').'<br>';
                            if( $distance_obj->distance_meters <= $distance_threshold ) {
                        ...
                        $diff = abs( floatval( $v1['heading'] ) - floatval( $v2['heading'] ) );
                        ...
                        // echo '<pre>else<br>';print_r($row); echo '</pre>';
                        ...
                        $sql = "update ".$event_table_mother." set ais_signal = '".$vessel1_signal."' where id='".$row['id']."'";
                        $mysqli->query($sql);

                        $sql = "update ".$event_table_daughter." set ais_signal = '".$vessel2_signal."' where id='".$row['did']."'";
                        $mysqli->query($sql);
                    ...
                    $result2->free();
                        ...
                        //         echo '<pre>';print_r($vehicel_1); echo '</pre>';
                        //         echo '<pre>';print_r($vehicel_2); echo '</pre>';
                        //         echo '<pre>';print_r($vehicel_1_detail); echo '</pre>';
                        //         echo '<pre>';print_r($vehicel_2_detail); echo '</pre>';
                        //         $vess1 = [
                        //             'name' => $vehicel_1_detail['name'],
                        //             'mmsi' => $vehicel_1_detail['mmsi'],
                        //             'length' => $vehicel_1_detail['length'],
                        //             'deadweight' => $vehicel_1_detail['deadweight'],
                        //             'speed' => $vehicel_1['speed'],
                        //             'navigation_status' => $vehicel_1['type_specific']
                        //         ];

                        //         $vess2 = [
                        //             'name' => $vehicel_2_detail['name'],
                        //             'mmsi' => $vehicel_2_detail['mmsi'],
                        //             'length' => $vehicel_2_detail['length'],
                        //             'deadweight' => $vehicel_2_detail['deadweight'],
                        //             'speed' => $vehicel_2['speed'],
                        //             'navigation_status' => $vehicel_2['type_specific']
                        //         ];
                        //         print_r( determineMother($vess1, $vess2, $vehicel_1_detail['mmsi'], $vehicel_2_detail['mmsi']));
                        //        // determineMother($vessel1, $vessel2, $mmsi1, $mmsi2)
                        //     }
                    ...
                    //                     coastalynk_log_entry($event_id, 'Added the parent vessel '.$v1['name'].' with MMSI '.$v1['mmsi'].' and IMO '.$v1['imo'].' to the event.', 'sts');
                    //                 }
                    //             }
                    ...
                    //                     `ais_signal`,`deadweight`,`gross_tonnage`,distance, last_updated, remarks, event_percentage, cargo_category_type, `risk_level`, `stationary_duration_hours`, `proximity_consistency`, `data_points_analyzed`, `operationmode`, `status` )
                    //                     VALUES (
                    ...
                    //                     echo "Error: " . $sql . "\n" . $mysqli->error;
                    //                 } else {
                    //                     $insert_id = $array_ids[] = $event_id;
                    //                     $array_uidds[] = [ $v1['uuid'], $v2['uuid'] ];
                    //                     coastalynk_log_entry($insert_id, 'STS Daughter vessel Between '.$v1['name'].' and '.$v2['name'].': '.$remarks, 'sts-daughter');
                    //                     $total_allowed++;
                    ...
                    //                         $mysqli->query($sql);
                    //                         coastalynk_log_entry($event_id, 'STS Between '.$v1['name'].' and '.$v2['name'].': Email sent successfully!', 'sts');
            ...
            // echo '<pre>';
            // print_r($vessel_array);
            // echo '</pre>';
            // $multiResult = $detector->processVessels($vessel_array);
            // echo "<pre>";
            // print_r($multiResult);
            // echo "</pre>";
        ...
        if( count( $vessel_array ) > 1 ) {
            echo '<pre>';print_r($vessel_array); echo '</pre>';
            ...
            for( $i=0; $i<count($vessel_array_keys); $i++) {
                ...
                if( ! $mother_ship ) {
                    ...
                    echo '<pre>';print_r($mother_ship);echo '</pre>';
                }
            }
            ...
            foreach( $vessel_array as $mmsi => $vessel_data ) {
                ...
                echo '<pre>Vessel Data Start<BR>'; print_r($vehicel_1);
                echo '<pre>Vessel Detail Data Start<BR>'; print_r( $vehicel_1_detail);echo '</pre>';

                echo '<pre>Vessel Data Start<BR>'; print_r($vehicel_2);
                echo '<pre>Vessel Detail Data Start<BR>'; print_r( $vehicel_2_detail);echo '</pre>';
                ...
                // $mother_vessel_number = $mother_ship = 0;
                // if( $cached_detail_data[$mmsi] && $vehicel_2_detail ) {
                //     if( floatval( $cached_detail_data[$mmsi]['gross_tonnage'] ) > 0 && floatval( $vehicel_2_detail['gross_tonnage'] ) > 0 ) {
                //         if( floatval( $cached_detail_data[$mmsi]['gross_tonnage'] ) > floatval( $vehicel_2_detail['gross_tonnage'] ) ) {
                //             $mother_vessel_number = $mother_ship = 1;
                //             $zone_ship  = $vehicel_1['name'];
                //         } else {
                //             $mother_vessel_number = $mother_ship = 2;
                //             $zone_ship  = $v2['name'];
                //         }
                ...
                //     if( $mother_ship == 0 ) {
                //         if( floatval( $vehicel_2['current_draught'] ) > 0 && floatval( $vehicel_2['current_draught'] ) > 0 ) {
                //             if( floatval( $vehicel_2['current_draught'] ) > floatval( $vehicel_2['current_draught'] ) ) {
                //                 $mother_vessel_number = $mother_ship = 1;
                //                 $zone_ship  = $vehicel_1['name'];
                //             } else {
                //                 $mother_vessel_number = $mother_ship = 2;
                ...
                //     if( $mother_ship == 0 ) {
                //         if( floatval( $cached_detail_data[$mmsi]['length'] ) > 0 && floatval( $vehicel_2_detail['length'] ) > 0 ) {
                //             if( floatval( $cached_detail_data[$mmsi]['length'] ) > floatval( $vehicel_2_detail['length'] ) ) {
                //                 $mother_vessel_number = $mother_ship = 1;
                //                 $zone_ship  = $vehicel_1['name'];
                //             } else {
                //                 $mother_vessel_number = $mother_ship = 2;
                ...
                if( $event_id == 0 ) {
                   ...
                   echo '<br>'. $sql = "INSERT INTO $event_table_mother (
                                `uuid`,`name`,`mmsi`,`imo`,`country_iso`,`type`,`type_specific`,`lat` ,
                                `lon` ,`speed` ,`navigation_status`,`draught`,
                                `last_position_UTC`, `deadweight`, `gross_tonnage`, `port` ,
                                `port_id`, `distance`, `event_ref_id`, `zone_type`, `zone_ship`, `zone_terminal_name` ,
                ...
                if( $event_id > 0 ) {
                    ...
                    echo '<br>'.$sql = "INSERT INTO $event_table_daughter (
                        `event_id`,`uuid`,`name`,`mmsi`,`imo`,`country_iso`,`type`,`type_specific`,`lat`,
                        `lon`,`speed`,`navigation_status`,`draught`,`last_position_UTC`,
                        `ais_signal`,`deadweight`,`gross_tonnage`,distance, last_updated, remarks, event_percentage, cargo_category_type, `risk_level`, `stationary_duration_hours`, `proximity_consistency`, `data_points_analyzed`, `operationmode`, `status` )
                        VALUES (
                    ...
                    if ($mysqli->query( $sql ) !== TRUE) {
                        ...
                        // $coastalynk_sts_body = str_replace( "[vessel2_navigation_status]", $vehicel_2_navigation_status, $coastalynk_sts_body );
                        // $coastalynk_sts_body = str_replace( "[vessel2_draught]", $vehicel_2_current_draught, $coastalynk_sts_body );
...
if( count( $array_ids ) > 0 ) {
    ...
    $sql = "select e.id, e.id as did, d.event_id, e.deadweight as vessel1_deadweight, d.deadweight as vessel2_deadweight, e.port, e.zone_type, e.zone_ship, e.uuid as vessel1_uuid, e.zone_terminal_name, e.name as vessel1_name, d.name as vessel2_name, e.mmsi as vessel1_mmsi, d.mmsi as vessel2_mmsi, d.uuid as vessel2_uuid, e.end_date, e.draught as vessel1_draught, e.last_position_UTC as vessel1_last_position_UTC, d.last_position_UTC as vessel2_last_position_UTC, d.draught as vessel2_draught, e.completed_draught as vessel1_completed_draught, d.completed_draught as vessel2_completed_draught from ".$event_table_mother." as e inner join ".$event_table_daughter." as d on(e.id=d.event_id) where e.port='".$port_name."' and d.status = 'Detected' and e.id not in (".$array_ids_implode.") and e.is_complete = 'No';";
    ...
    if( $num_rows > 0 ) {
        while( $row = mysqli_fetch_array( $result4, MYSQLI_ASSOC ) ) {
            ...
            $sql = "update ".$event_table_daughter." set step = 1, draught_change = '".$draught_diff."', status = 'Completed',outcome_status = '".$outcome_status."',operationmode = '".$current_status."',event_desc = '".$event_desc."',is_complete = 'Yes', ais_signal = '".$vessel2_signal."', estimated_cargo = '".$estimated_cargo."', completed_draught = '".floatval( $v2_current_draught)."', end_date = NOW(), last_updated = NOW() where is_complete='No' and id='".$row['did']."'";
            $mysqli->query($sql);
